// export function checkTranscript(transcriptLog, appendToOverlay) {
...
//   const latest = transcriptElements[transcriptElements.length - 1].innerText.trim();

//   if (!transcriptLog.has(latest)) {
//     transcriptLog.add(latest);
//     appendToOverlay(latest);
//   }
// }
...
//   if (transcriptElements.length < 2) return;
...
//   // Only process the previous index when a new index is seen
//   if (!prevIndex || prevIndex === lastFinalizedIndex) return;
...
//   if (!text || transcriptLog.has(prevIndex)) return;
